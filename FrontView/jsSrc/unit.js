...
function PreviewPic(path){
  if (path == ""){
		...
		window.open("viewimage.asp?p=" + path,"preImg","");
  }
}

function switchTab1(tabsize,prefix,index){
  for (var i=0;i<tabsize;i++){
    $(prefix + i).addClassName('vHid');
    $(prefix + 'btn' + i).removeClassName('select');
  }
  $(prefix + index).removeClassName('vHid');
  $(prefix + 'btn' + index).addClassName('select');
}
