...
function Marquee(){
	...
	if(!this.ID){
		alert("paramter \"" + arguments[0] + "\" init error \r\nplease check \"ID\"!");
		this.ID = -1;
		return;
	};
	...
	if (this.Direction==0 || this.Direction==1){
		var h = $(this.ID).down().getHeight();
		if (h <= this.Height){
			isstart = false;
		}
	}else{
		var w = $(this.ID).down().getWidth();
		if (w <= this.Width){
			isstart = false;
		}
	}
	if(arguments.length >= 7 && isstart)this.Start();
};
Marquee.prototype.Start = function(){
	...
	if(this.Height == 0)this.Height = $(this.ID).getHeight();//parseInt(this.ID.offsetHeight);
