...
function validateBefore() {
  ...
  } else if (valType == 'OTHER') {//如果类型是自定义，则获取自定义的验证字符串
...
function beforeSubmit() {
  ...
  $.each($("[valType]"), function (i, n) {
    //清除可能已有的提示信息
    $(n).poshytip('hide');
    if ($(n).attr("valType") == 'PASSW') {
      if ($(n).val() != '') {
        //显示tips			
        pass = $(n).val();
      }
      else { $(n).poshytip('show'); }
